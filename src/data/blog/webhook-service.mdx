...
2025/07/19 參加了 **Tech Book Community** 的線下小聚，由 Zet 大大分享 **Render Props** 的概念。在聽完的當下覺得 [Zet]() 大大不愧是暢銷書的作者，將複雜的概念解釋得淺顯易懂，讓人受益良多。
...
在 **Class Component** 的年代，`Component` 是定義狀態的唯一載體，因此如果想要共用邏輯，通常可以透過 **HOC (Higher Order Component)** 或是 **Render Props** 來達成，到了 **Hooks** 時代，共用邏輯的方式就被 **Hooks** 取代。

### HOC (Higher Order Component)

**HOC** 是一個接受組件作為參數並返回新組件的高階函數。它透過包裝原始組件來注入共用邏輯，讓原始組件可以獲得額外的功能而不需要修改自身的程式碼。
...
**Render Props** 是指組件接收一個函數作為 `prop`，這個函數會返回 **React** 元素，組件內部會呼叫這個函數並將內部狀態或邏輯作為參數傳遞給它。這種模式讓組件可以將自己的邏輯分享給其他組件，同時保持渲染的彈性。
